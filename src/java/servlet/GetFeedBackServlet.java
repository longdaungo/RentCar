...
public class GetFeedBackServlet extends HttpServlet {
     ...
     */
    private static final Logger LOGGER = Logger.getLogger(GetFeedBackServlet.class);
    private final String VIEWRATING = "ViewRating";
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        PrintWriter out = response.getWriter();
        String carID = request.getParameter("carID");
        String url = VIEWRATING;
        ServletContext appScope = request.getServletContext();
        Map<String, String> listResource = (Map<String, String>) appScope.getAttribute("LISTRESOURCE");
        try {
            /* TODO output your page here. You may use following sample code. */
            FeedBackDAO dao = new FeedBackDAO();
            List<FeedBackDTO> list = dao.getListFeedBack(carID);
            int averageRating = dao.getAverage(carID);
            request.setAttribute("LISTFEEDBACK", list);
            request.setAttribute("AVARAGE", averageRating);
        }
